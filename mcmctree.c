...
char *fossils[] = { " ", "L", "U", "B", "G", "SN", "ST", "S2N", "F" };
int npfossils[] = { 0,   4,   2,   4,   2,   3,    4,     7 ,   2};
...
int main(int argc, char *argv[])
{
   ...
   if (data.pfossilerror[0] == 0 && !stree.nodes[stree.root].fossil) {
      ...
      if (stree.RootAge[3] == -1) {
        stree.nodes[stree.root].fossil = GAMMA_F;
      } else if (stree.RootAge[3] == -2) {
        stree.nodes[stree.root].fossil = UNIF_F;
      } else {
   ...
   if (stree.nodes[stree.root].fossil == UNIF_F) {
     if (stree.nodes[stree.root].pfossil[0] < com.tipDateOld) {
       printf("\nIn backward time (the last sample time is time 0)\n");
       printf("Bounds on root age: %f, %f\n", stree.nodes[stree.root].pfossil[0], stree.nodes[stree.root].pfossil[1]);
       printf("Oldest sample time: %f\n", com.tipDateOld);
       error2 ("With a uniform prior on the root, the bounds should not overlap with the sample times");
     }
   }
...
int GetMem(void)
{
   ...
   */
   ...
   mcmc.nsteplength = mcmc.nsteplength + stree.nLatent;
   ...
   mcmc.steplengthOffset[1] = s - 1 + stree.nLatent;             /* mu[] & sigma2[] */
...
int GetOptions(char *ctlf)
{
            ...
            for (iopt = 0; iopt < nopt; iopt++) {
               if (strncmp(opt, optstr[iopt], 8) == 0) {
                  ...
                  case (17):
                    ...
                    if (pline = strstr(line, "G(")) {
                      sscanf(pline + 2, "%lf,%lf", &stree.RootAge[0], &stree.RootAge[1]);
                      stree.RootAge[3] = -1;

                    } if (pline = strstr(line, "F(")) {
                      sscanf(pline + 2, "%lf,%lf", &stree.RootAge[0], &stree.RootAge[1]);
                      stree.RootAge[3] = -2;
                    }
                     break;
   ...
   if (com.latentData && !com.TipDate) error2("Need tip dating to use latent file");
...
int GetInitialsTimes(void)
{
   ...
   if (com.TipDate && (stree.nodes[stree.root].fossil != UNIF_F && stree.nodes[stree.root].fossil != GAMMA_F && stree.nodes[stree.root].fossil != BOUND_F) )
   ...
   for (i = s; i < s21; i++) {
      ...
      case (GAMMA_F):
         ...
         if (com.latentData && i == stree.nspecies) {
           tmin[i] = tmin[i] + com.tipDateOld;
           tmax[i] = tmax[i] + com.tipDateOld;
         }
         break;
      case (SKEWN_F):
      ...
      case (S2N_F):
         ...
         break;
      case (UNIF_F):  tmin[i] = p[0]; tmax[i] = p[1]; break;
      }
   ...
   if (com.latentData) {
     for (int tip = 0; tip < com.ns; tip++) {
       /* The tip is latent */
       if (stree.nodes[tip].latent) {
         tmax[tip] = stree.nodes[stree.nodes[tip].father].age;
   ...
   for(int index = 0; index < s; index ++) {
     printf("Node %f \n", stree.nodes[index].age);
   }

   double newRoot = 0;
   double oldRoot = stree.nodes[com.ns].age;
   if (com.latentData) {
     if ( (stree.nodes[com.ns].fossil == UNIF_F) && (oldRoot < stree.nodes[com.ns].pfossil[0] || oldRoot > stree.nodes[com.ns].pfossil[1])) {
       newRoot = (stree.nodes[com.ns].pfossil[0] + stree.nodes[com.ns].pfossil[1]) / 2;

     } else if ((stree.nodes[com.ns].fossil == GAMMA_F)  && (oldRoot < com.tipDateOld )) {
       newRoot = (stree.nodes[com.ns].pfossil[0] / stree.nodes[com.ns].pfossil[1]) + com.tipDateOld;
     }
   }

   if (newRoot > 0) {
      stree.nodes[com.ns].age = newRoot;
     for (k = 0; k < stree.nnode; k ++ ) {
         if (stree.nodes[k].age > com.tipDateOld && k != stree.nspecies) {
             stree.nodes[k].age = (stree.nodes[k].age - com.tipDateOld) * (newRoot - com.tipDateOld) / (oldRoot - com.tipDateOld) + com.tipDateOld;
         }
       }
  }
   ...
   */
   ...
   if (com.latentData) {
     for (i = 0; i < stree.nspecies; i++) {
        if(stree.nodes[i].latent) {
          if (firsttime && fout)  fprintf(fout, "\tt_n%d", i + 1); /* A: Might need to change label */
          x[np++] = stree.nodes[i].age;
        }
     }
   }
...
double lnpriorTimesBDS_Approach1Latent(void)
{
   ...
   */
   ...
   if (psi == 0 && fabs(lambda - mu) < 1e-20) {
      ...
      for (j = stree.nspecies; j < stree.nnode; j++)
         if (j != stree.root) {
		 if (stree.nodes[j].latent) {
            		c2 = 1 + rho*lambda*stree.nodes[j].latentAge;
      ...
      for (j = stree.nspecies; j < stree.nnode; j++)
         if (j != stree.root) {
		 if (stree.nodes[j].latent) {
            		e = exp((mu - lambda)*stree.nodes[j].latentAge);
      ...
      for (j = stree.nspecies; j < stree.nnode; j++) {
         if (j == stree.root) continue;
         for (k = stree.nodes[j].sons[0]; k >= stree.nspecies; k = stree.nodes[k].sons[1]);
	 if (stree.nodes[k].latent) {
         	z0 = stree.nodes[k].latentAge;
	 } else {
         	z0 = stree.nodes[k].age;
	 }
         /* printf("node %2d z[%d] = %7.4f ", j+1, k+1, z0); */
   ...
   /* prior on t1 */
   if (stree.nodes[stree.nspecies].fossil != BOUND_F && stree.nodes[stree.nspecies].fossil != GAMMA_F && stree.nodes[stree.nspecies].fossil != UNIF_F )
      error2("Only bounds for the root age are implemented.");
   lnp += lnptCalibrationDensity(t1, stree.nodes[stree.nspecies].fossil, stree.nodes[stree.nspecies].pfossil);
...
/* This is used to check the proposal step. It has a prior on the root age, but
   not the branching times. To check, look at distribution of branching times
   without data. */
double lnpriorTimesBDS_Approach1_OnlyRoot(void) {

   double lnp = 0, t, t1 = stree.nodes[stree.root].age;
   lnp += lnptCalibrationDensity(t1, stree.nodes[stree.nspecies].fossil, stree.nodes[stree.nspecies].pfossil);
...
/*double lnpriorTimesRootShiftGamma(void) {
  double rootAge = stree.nodes[stree.root].age - com.tipDateOld;

  assert(rootAge > 0);

  if (stree.nodes[stree.nspecies].fossil != GAMMA_F)
     error2("Must have gamma prior on root age to infer latent history");

  double lnp = lnptCalibrationDensity(rootAge, stree.nodes[stree.nspecies].fossil, stree.nodes[stree.nspecies].pfossil);
...
double lnptCalibrationDensity(double t, int fossil, double p[7])
{
  ...
  case (UNIF_F):
      if (t > a && t < b) {
        ...
        lnpt = -99999;
        //error2("Likelihood if zero if root is not in the uninform interval. The log likelihood is undefined.");
      }
   }
   return(lnpt);
}
...
double lnpriorTimes(void)
{
   ...
   */
   ...
   if(com.latentData) {
     ...
     // if (1)        lnpt = lnpriorTimesBDS_Approach1Latent();
     if (stree.nodes[stree.nspecies].fossil == GAMMA_F){
       double rootAge = stree.nodes[stree.root].age - com.tipDateOld;
       assert(rootAge > 0);

        lnpt = lnptCalibrationDensity(rootAge, stree.nodes[stree.nspecies].fossil, stree.nodes[stree.nspecies].pfossil);
      } else if (stree.nodes[stree.nspecies].fossil == UNIF_F) {
        lnpt = lnptCalibrationDensity(stree.nodes[stree.root].age, stree.nodes[stree.nspecies].fossil, stree.nodes[stree.nspecies].pfossil);
      } else {
        error2("Must have gamma or uniform prior on root age to infer latent history");
      } 

   } else if (stree.nfossil == 0 || com.TipDate) {
     //if (1)        lnpt = lnpriorTimesBDS_Approach1_OnlyRoot();
...
int UpdateTimesLatent (double *lnL, double steplength[], char accept[])
{
   ...
   */
   ...
   double lnacceptance = 0, lnLd, lnpDinew[NGENE], lnpTnew, lnpRnew = -1;
   double* oldTimes = malloc(sizeof(double) * stree.nnode);

   if (stree.duplication) {  /*A: should be false with latent tip dating */
   ...
   for (is = stree.nspecies; is < stree.nnode; is++) {
      ...
      //printf("Before Likelihood %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f\n", data.lnpDi[0], stree.nodes[0].age, stree.nodes[1].age, stree.nodes[2].age, stree.nodes[3].age, stree.nodes[4].age, stree.nodes[5].age, stree.nodes[6].age,com.pi[2], com.pi[1], com.pi[3],  com.pi[0],  com.kappa, data.rgene[0]);
      ...
      /* Since the root ages are proposed and then the other nodes are changed with
      a rubber band algorithm, there tip ages are only constrained by the prior.
      These bounds constrain the root age to the range for which the prior has
      positive density. */
      if (is == stree.root && stree.nodes[stree.nspecies].fossil == UNIF_F) {
        tmax = stree.nodes[is].pfossil[1];
      //  tmin = stree.nodes[is].pfossil[0];
      }
      ...
      lnacceptance += lnLd;
      //printf("After Likelihood %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f\n", lnpDinew[locus-1], stree.nodes[0].age, stree.nodes[1].age, stree.nodes[2].age, stree.nodes[3].age, stree.nodes[4].age, stree.nodes[5].age, stree.nodes[6].age,com.pi[2], com.pi[1], com.pi[3],  com.pi[0],  com.kappa, data.rgene[0]);
      //printf("lnLd %f\n", lnLd);
   ...
   for (is = 0; is < stree.nspecies; is++) {
      ...
      if (!stree.nodes[is].latent) continue;
      //printf("Before latent %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f\n", data.lnpDi[0], stree.nodes[0].age, stree.nodes[1].age, stree.nodes[2].age, stree.nodes[3].age, stree.nodes[4].age, stree.nodes[5].age, stree.nodes[6].age,com.pi[2], com.pi[1], com.pi[3],  com.pi[0],  com.kappa, data.rgene[0]);

      t = stree.nodes[is].age;
      dad = stree.nodes[is].father;
      tmin = stree.nodes[is].latentAge;
      tmax = (is == stree.root ? 1e9 : stree.nodes[dad].age);
      if (com.latentBound && (tmax > com.latentBoundDate)) {
	tmax = com.latentBoundDate;
      }
      ...
      y = log(t);
      e = steplength[stree.nspecies - 1 + latentIndex];
      if (e <= 0) error2("steplength = 0 in UpdateTimes");
      ...
      lnacceptance += lnLd;
      //printf("After latent %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f\n", lnpDinew[locus-1], stree.nodes[0].age, stree.nodes[1].age, stree.nodes[2].age, stree.nodes[3].age, stree.nodes[4].age, stree.nodes[5].age, stree.nodes[6].age,com.pi[2], com.pi[1], com.pi[3],  com.pi[0],  com.kappa, data.rgene[0]);
      //printf("lnLd %f\n", lnLd);

      if (lnacceptance >= 0 || rndu() < exp(lnacceptance)) {
         accept[stree.nspecies - 1 + latentIndex] = (char)1;
         data.lnpT = lnpTnew;
         if (com.clock == 3) data.lnpR = lnpRnew;
         for (locus = 0; locus < data.ngene; locus++)
            data.lnpDi[locus] = lnpDinew[locus];
...
int UpdateTimesLatentRubberBand (double *lnL, double steplength[], char accept[])
{
   /* This updates the node ages in the master species tree stree.nodes[].age,
      stree.duplication:
      stree.nodes[i].label = 0:  usual node, which may and may not have calibration.
   ...
   */
   ...
   for (is = stree.nspecies; is < stree.nnode; is++) {
      ...
      if (is == stree.root && stree.nodes[stree.nspecies].fossil == UNIF_F) {
        ...
        /* The gamma distribution does not have an upper bound */
      } else if (is == stree.root && stree.nodes[stree.nspecies].fossil == GAMMA_F) {
        tmin = com.tipDateOld;
      ...
      if (is != stree.nspecies) {
        ...
        /* New proposal for root */
      } else {
        lnacceptance = 0;
        e = steplength[is - stree.nspecies];
        if (e <= 0) error2("steplength = 0 in UpdateTimes");

        tnew = t + e * rndSymmetrical();

        /* If the proposed time is negative */
        if (tnew < 0) {
         /* Shift the times to all be positive, then shift them back*/
          double tempT = abs (tnew) + 1;
          tnew = exp(reflect(log(tnew + tempT), log(tempT + tmin), log(tmax + tempT))) - tempT;

        } else {
          tnew = exp(reflect(log(tnew), log(tmin), log(tmax)));

        }


        stree.nodes[is].age = tnew;

	      int numUpdate = 0;

        /* Find all nodes (and latency times) that are older than the youngest sampling time */
        /* Also, need to update times */
       for (int k = 0; k < stree.nnode; k ++ ) {
          oldTimes[k] = stree.nodes[k].age;
          if (stree.nodes[k].age > com.tipDateOld && k != stree.nspecies) {
              stree.nodes[k].age = (stree.nodes[k].age - com.tipDateOld) * (tnew - com.tipDateOld) / (t - com.tipDateOld) + com.tipDateOld;
	             numUpdate++;
          }
        }
       /* I believe this does not have the proposal ratio */
       lnacceptance = numUpdate * log((tnew - com.tipDateOld) / (t - com.tipDateOld));
      }
      ...
      if (lnacceptance >= 0 || rndu() < exp(lnacceptance)) {
        ...
        if (is == stree.nspecies) {
          ...
          /* Reset times changed by rubber band */
          for (int k = 0; k < stree.nnode; k ++ ) {
              stree.nodes[k].age = oldTimes[k];
           }
        }
...
int mixingTipDateLatent(double *lnL, double steplength, char *accept)
{
   /* This increases or decreases all node ages in a 1-D move, applied to TipDated data.
      1 September 2011, Aguas de Lindoia
      Znote 2014-11-21:  Please check that changemu=0 is correct and gives the same result as changemu=1.
   */
   ...
   minages = (double*)malloc((s + stree.nLatent) * 2 * sizeof(double));
   if (minages == NULL) error2("oom");
   memset(minages, 0, (s + stree.nLatent) * 2 * sizeof(double));
   x = minages + s + stree.nLatent;
   x[0] = 1;
   ...
   latentIndex = 0;
   for (j = 0; j < s; j++) {
     /* Should scale branches with latent times */
     if (stree.nodes[j].latent) {
       tz = stree.nodes[j].latentAge;
       minages[s + latentIndex] = tz;
       latentIndex++;
   ...
   lnacceptance = lnc = steplength*rndSymmetrical();
   c = exp(lnc);
   // Does this line match the algorithm?
   stree.nodes[stree.root].age = minages[0] + (stree.nodes[stree.root].age - minages[0]) * c;
   for (j = s; j < s*2-1; j++) {
      if (j == stree.root) continue;
      jj = j - s;
   ...
   if (com.clock > 1) {              /* rate-drift models */
     error2("Drift rate models mixing not implemented for latent data");
      ndivide += g*(s * 2 - 2);
      for (i = 0; i < stree.nnode; i++)
         if (i != stree.root)
            for (j = 0; j < g; j++)
...
int DescriptiveStatisticsSimpleMCMCTREE(FILE *fout, char infile[])
{
   ...
   for (j = 0; j < stree.nspecies; j++) {
     if (stree.nodes[j].latent) {
       ...
       jj = SkipC1 + stree.nspecies - 1 + latentIndex;
       sprintf(nodes[j].annotation, "[&95%%HPD={%.6g, %.6g}]\0", xHPD025[jj], xHPD975[jj]);
       latentIndex = latentIndex + 1;
     }
   }
   ...
   for (j = SkipC1; j < p; j++) {
      ...
      if (j < SkipC1 + stree.nspecies - 1 + stree.nLatent) {
         printf("  (Jnode %2d)", 2 * stree.nspecies - 1 - 1 - j + SkipC1);
         if (com.TipDate)
            printf(" time: %7.3f (%6.3f, %6.3f)", com.TipDate - mean[j] * com.TipDate_TimeUnit,
               com.TipDate - xHPD975[j] * com.TipDate_TimeUnit, com.TipDate - xHPD025[j] * com.TipDate_TimeUnit);
      }
      printf("\n");
   }
...
int MCMC(FILE* fout)
{
   ...
   */
   ...
   if (com.latentData) {
     if (com.clock == 1)
        printf("paras: %d times, %d latent times, %d mu, (& kappa, alpha)\n", stree.nspecies - 1, stree.nLatent, data.ngene);
     else
        printf("paras: %d times, %d latent times, %d mu, %d sigma2 (& rates, kappa, alpha)\n", stree.nspecies - 1, stree.nLatent, data.ngene, data.ngene);
   ...
   for (ir = -mcmc.burnin; ir < mcmc.sampfreq*mcmc.nsample; ir++) {
      ...
      if (com.latentData) {
        UpdateTimesLatent(&lnL, mcmc.steplength, mcmc.accept);
  ...
  for (i = 0; i < stree.nspecies; i++) {
    if (nodes[i].latent) {
      j = stree.nspecies - 1 + latentIndex;
      nodes[i].age = mx[j];
      latentIndex++;
    }
  }
   ...
   FreeMem();
   if (com.latentData) {
	   if (stree.nodes[stree.root].fossil == GAMMA_F) {
	   	printf("\nRoot prior: Gamma (%f, %f)\n", stree.nodes[stree.root].pfossil[0], stree.nodes[stree.root].pfossil[1]); 
	   } else if (stree.nodes[stree.root].fossil == UNIF_F) {
	   	printf("Root prior: Unif (%f, %f)\n", stree.nodes[stree.root].pfossil[0], stree.nodes[stree.root].pfossil[1]); 
	   }
   } else {
